/*
 * fmjam Custom - FM/RDS transmitter for the Raspberry Pi
 *
...
int tx(uint32_t carrier_freq)
{
    ...
    float ppm = 0.0; // No frequency correction needed for static broadcast
    ...
    printf("Divider is %.4f (%d + %d*2^-12) [nominal 1096.4912].\n",
           divider, idivider, fdivider);
    ...
    printf("Starting static broadcast on %3.1f MHz.\n", carrier_freq / 1e6);
    printf("This will generate harmonics across the spectrum.\n");
    ...
    for (;;)
    {
        ...
        while (free_slots >= 1)
        {
            // Generate pseudo-random deviation for broadband noise
            // This creates spectral content that spreads via harmonics
            int random_dev = (rand() % 2000) - 1000;
            
            // Or for pure carrier with clean harmonics, use:
            // int random_dev = 0;
            
            // Write frequency sample
            ctl->sample[last_sample++] = (0x5A << 24 | freq_ctl) + random_dev;
...
{
    uint32_t carrier_freq = 18000000; 

    if (argc > 1) {
        carrier_freq = 1e6 * atof(argv[1]);
        if (carrier_freq < 1e6 || carrier_freq > 200e6) {
            fprintf(stderr, "Frequency must be between 1 and 200 MHz\n");
            fprintf(stderr, "Usage: %s [frequency_in_MHz]\n", argv[0]);
            fprintf(stderr, "Example: %s 22.0\n", argv[0]);
            fprintf(stderr, "\nRecommended frequencies for FM band harmonics:\n");
            fprintf(stderr, "  22.0 MHz - harmonics at 88, 110 MHz\n");
            fprintf(stderr, "  18.0 MHz - harmonics at 90, 108 MHz\n");
            fprintf(stderr, "  21.875 MHz - harmonic at 87.5 MHz\n");
            return 1;
        }
    }

    printf("\nFmJam\n");
    printf("Carrier frequency: %.3f MHz\n", carrier_freq / 1e6);
    printf("Press Ctrl+C to stop\n\n");

    srand(time(NULL));

    int errcode = tx(carrier_freq);
